...
public class GenProfile {
    ...
    private static final Logger LOGGER =
        LogManager.getLogger(GenProfile.class);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    LOGGER.error("Unable to close training file '" + file.getName() + "'", e);
                }
            }

        } catch (IOException e) {
            throw new LangDetectException(ErrorCode.CantOpenTrainData,
                "Can't open training database file '" + file.getName() + "'");
        } finally {
            try {
                if (is != null) is.close();
            } catch (IOException e) {
                LOGGER.error("Unable to close training file input stream for '" + file.getName() + "'", e);
            }
        }
        return profile;
    }
    ...
    public static LangProfile loadFromText(String lang, File file) throws LangDetectException {
        ...
        try {
            ...
            throw new LangDetectException(ErrorCode.CantOpenTrainData,
                "Can't open training text file '" + file.getName() + "'");
        } finally {
            try {
                if (is != null) is.close();
            } catch (IOException e) {
